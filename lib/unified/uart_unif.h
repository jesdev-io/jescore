...
static inline int32_t uart_unif_init(uint32_t baud, uint32_t rx_buf_len, uint32_t tx_buf_len, void* args){
...
static inline int32_t uart_unif_write(const char *msg) {
    uint16_t len = strlen(msg);
    return uart_write_bytes(BASE_UART, msg, len);
}

static inline int32_t uart_unif_writef(const char *format, ...) {
...
static inline int32_t uart_unif_read(uint8_t* buf, uint32_t len, uint32_t timeout){
    return uart_read_bytes(BASE_UART, buf, len, (TickType_t)timeout);
}

static inline int32_t uart_unif_flush_inp(void){
    return uart_flush_input(BASE_UART);
}

static inline int32_t uart_unif_flush(void){
    return uart_flush(BASE_UART);
}

#elif defined(__AVR__)

static inline int32_t uart_unif_write(uint8_t* msg, uint32_t len, uint32_t timeout){
